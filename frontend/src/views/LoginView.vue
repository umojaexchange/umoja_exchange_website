<template>
  <div class="login-bg">
    <!-- Orbs (no blur — use opacity + color instead) -->
    <div class="orb orb1" />
    <div class="orb orb2" />
    <div class="orb orb3" />
    <!-- grid-pattern div removed -->
    ...
    <div class="login-wrap">
      ...
      <div class="login-card animate-up" style="animation-delay:.1s">
        ...
        <div style="display:flex;gap:6px;justify-content:center;margin-top:20px">
          <button v-for="l in ['en','sw']" :key="l" @click="setLang(l)" class="lang-pill"
                  :class="{ active: lang===l }">
...
/* ── Orbs — NO blur filter, use will-change for GPU compositing ── */
.orb {
  position: absolute;
  border-radius: 50%;
  opacity: 0.18;
  /* will-change lifts the orb onto its own compositor layer    */
  /* so the animation never triggers a repaint of the page      */
  will-change: transform;
  animation: float 10s ease-in-out infinite;
  /* removed: filter:blur() — this was causing the freeze       */
}
.orb1 { width:500px; height:500px; background:radial-gradient(circle, #FACC15 0%, transparent 70%); top:-150px; right:-150px; animation-delay:0s; }
.orb2 { width:400px; height:400px; background:radial-gradient(circle, #3B82F6 0%, transparent 70%); bottom:-120px; left:-120px; animation-delay:-4s; }
.orb3 { width:280px; height:280px; background:radial-gradient(circle, #8B5CF6 0%, transparent 70%); top:40%; left:45%; animation-delay:-7s; }

/* Only translate — never trigger layout or paint */
@keyframes float {
  0%,100% { transform: translateY(0px); }
  50%      { transform: translateY(-24px); }
}

/* grid-pattern removed entirely */
...
/* ── Card — NO backdrop-filter (huge perf cost) ──────────────── */
.login-card {
  width: 100%;
  /* solid semi-transparent bg instead of backdrop-filter:blur() */
...
.login-btn {
  ...
  box-shadow: 0 4px 20px rgba(250,204,21,0.28); margin-top:4px;
  /* will-change keeps hover transform on compositor layer */
  will-change: transform;
}
...
.lang-pill.active {
  background:var(--yellow,#FACC15); color:#0F0F0F;
